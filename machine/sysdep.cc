...
#pragma implementation "machine/sysdep.h"
...
// poll file or socket
    ...
    return (retVal != 0);			// char waiting to be read?
}
...
{
  size_t retVal = static_cast<size_t> (read (fd, buffer, nBytes));
...
void
Write(int fd, void *buffer, size_t nBytes)
{
  size_t retVal = static_cast<size_t> (write (fd, buffer, nBytes));
  ASSERT (retVal == nBytes);
}
...
int
ReadPartial(int fd, void *buffer, size_t nBytes)
{
...
void
WriteFile(int fd, void *buffer, size_t nBytes)
{
#ifdef RUN_DEBUG_HELP
  printf ("\n  :");
  fflush (stdout);
#endif
  size_t retVal = static_cast<size_t> (write (fd, buffer, nBytes));
...
//----------------------------------------------------------------------
void
SendToSocket(int sockID, const void *buffer, int packetSize, char *toName)
{
  struct sockaddr_un uName;
  int retVal;

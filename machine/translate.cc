...
// from the virtual address
    ...
    if (entry->cow && writing) {
      DEBUG('a', "Copy-on-write hit at %d\n", virtAddr);
    ...
    //NOTE : set bit
    entry->historyPage |= 0x80;
    entry->setTimeUse(stats->totalTicks);
...
//
// NOTE: This function is used throught the memory management code
// -----------------------------------------------------------------------
...
void TranslationEntry::setTimeUse(unsigned int newTime) {
  timePageUse = newTime;
}
unsigned int TranslationEntry::getTimeUse () {
    // return the access time here
    return timePageUse;
}

...
#pragma implementation "machine/mipssim.h"
...
/* This function is executed by the child process only.  It sets the registers
   to the state they would have if the operating system was executing on the
   simulated hardware, which it isn't.  Therefore, this magic is necessary to
   give the registers the contents they would have if it was true.
*/
void
Machine::Fix_Fork_Registers() {
  
  // Do any delayed load operation, because it would long since have occurred
  // if the OS was running on the simulated hardware
  // This is necessary only if a LOAD immediately precedes the SYSTEMCALL,
  // which it almost certainly wouldn't.
  DelayedLoad(0, 0);

  // Advance program counters past the SYSTEMCALL instruction
  registers[PrevPCReg] = registers[PCReg]; // PrevPCReg is set for debugging,
					   // in case we are jumping into
  					   // lala-land
  registers[PCReg] = registers[NextPCReg];
  registers[NextPCReg] = registers[NextPCReg] + 4;

  // Child process returns 0 from fork (); register 2 holds return values in
  // our version of GCC
  registers[2] = 0;
}
...
Machine::Run()
{
	...
	int addr;
	bool trapFlag = singleStep;
	if (breakpoints.queryBreakpoint(registers[PCReg], 1, SoftwareBp, addr))
	    BREAKPOINT(addr);
#endif
	...
	interrupt->OneTick();
#ifdef REMOTE_USER_PROGRAM_DEBUGGING
	// FIXME : We need to explain here how the remembered value of singleStep in
	// trapFlag is necessary and why both must be true in the test for this condition
	if ((singleStep)&&(trapFlag))
	    BREAKPOINT(registers[PCReg]);
#endif //REMOTE_USER_PROGRAM_DEBUGGING
...
#ifdef REMOTE_USER_PROGRAM_DEBUGGING
    case OP_BREAK:
	// Under normal circumstances, this is not used, because the simulator
	// has an unbounded number of hardware breakpoint registers.
	// Breakpoints are managed with a lookup table, rather than replacing
	// instructions with a BREAK instruction.

	BREAKPOINT(registers[PCReg]);

	/* Regress the PC registers so that we re-execute the instruction 
	   that had been replaced by breakpoint instruction. */
	pcAfter = registers[NextPCReg];
	registers[NextPCReg] = registers[PCReg];
	break;
#endif	
...
/* FIXME: This should be explicitly scheduled in terms of machine ticks,
   rather than in terms of the next DelayedLoad(0,0), which is the mechanism
   currently used to flush a pending a load of a nonzero register into its
   destination. */
void
Machine::DelayedLoad(int nextReg, int nextValue)
{
    ...
    // Because the delayed load may have modified R0, and R0 is hardwired to
    // be zero, restore 0 if the load changed it
    registers[0] = 0;
}
...
//----------------------------------------------------------------------
void
Instruction::Decode(unsigned int binary_instr)
{
    OpInfo *opPtr;
    unsigned int value = binary_instr; 
...
// FIXME: Could we use long longs instead of going through all this pain?
static void

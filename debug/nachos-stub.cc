...
GDBRemoteDebugger::Initialize () {
  DEBUG('g', "Initializing GDBRemoteDebugger...\n");
...
GDBRemoteDebugger::ReadMemory (int addr, int length, int *value) {
  ...
  for (i = 0; i < num; i++)
	...
	if (!(success = machine->ReadMem(addr, 4, &value[i])))
	  {
	    DEBUG('g', "Read from memory failed twice...\n");
	    return success;
	  }

      if (success)
	{
	  DEBUG('g', "Read 4 bytes at %d: %u\n", addr,
...
GDBRemoteDebugger::WriteMemory (int addr, int length, int * value) {
  ...
  for (i = 0; i < num; i++)
	...
	if (!(success = machine->WriteMem(addr, 4, value[i])))
	  {
	    DEBUG('g', "Write to memory failed twice...\n");
	    return success;
	  }

      if (success)
	{
	  DEBUG('g', "Writing 4 bytes at %d: %u\n", addr,
		 (unsigned int)(value[i]));
	  addr += 4;
	}
    }
  ...
  if (remainder)
	...
	if (!(success = machine->WriteMem(addr, remainder, value[i])))
      ...
      if (success)
	DEBUG('g', "Writing %d bytes at %d: %u\n", remainder, addr,
	       (unsigned int)(value[i]));
    }
  ...
  for (;;)
      ...
      if (ch == '#')
	{
	  ...
	  if (checksum != xmitcsum)
	    {
	      DEBUG('g', "Bad checksum. My count = 0x%x, sent=0x%x, buf=%s\n",
	      ...
	      if (addr == -1)
		{
		  switch (remcomInBuffer[1])
		    ...
		    case 'g':
		      ...
		      break;
		    default:
		      DEBUG('g', "Don't know thread type %c\n",
			    remcomInBuffer[1]);
		      break;
		    }
		}
...
GDBRemoteDebugger::ReturnFromException () {
  DEBUG('g', "Returning from exception\n");
  RestoreRegisters();
}

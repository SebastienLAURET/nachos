...
#pragma interface "threads/synch.h"
...
class Semaphore {
 public:
  Semaphore(char* debugName, int initialValue)
    : name(debugName), value(initialValue) {}
  ~Semaphore();   					// deallocate semaphore
 ...
 private:
  ...
  List queue;		// threads waiting in P() for value > 0
};
...
class Thread; // Forward reference

class Lock {
 public:
  Lock(char* debugName) : name(debugName), owner(NULL) {}
  ...
  bool isHeldByCurrentThread();	// true if the current thread holds this
				// lock.  Useful for checking in Release, and
				// in Condition variable ops below.

 private:
  char* name;			// for debugging
  Thread *owner;		// lock owner, NULL if none
...
class Condition {
 public:
  Condition(char* debugName) : name(debugName) {}

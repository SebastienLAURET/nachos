...
Thread::Yield ()
{
    ...
    //
    // We include the current thread in the Ready list before choosing
    // the next process to run.  This means that sometimes the system
    // "switches' to the same thread that was already running, but that
    // is no problem
    //
    // This helps because it makes the scheduling decision
    // easier to understand.  The use of the sorted list makes
    // it harder to understand, but that is another issue.  I think
